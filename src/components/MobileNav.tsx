...
export function MobileNav({ activeTab, onTabChange }: MobileNavProps) {
  ...
  const tabBtn = (tab: { id: MobileTab; label: string; icon: React.ElementType }) => {
    const Icon = tab.icon;
    ...
    return (
      <button
        ...
        style={{ color: active ? 'hsl(142 55% 52%)' : 'hsl(0 0% 40%)' }}
      >
        <motion.div whileTap={{ scale: 0.85 }}>
          <Icon className="h-[18px] w-[18px]" />
        </motion.div>
        <span className="text-[8px] font-mono uppercase tracking-widest">{tab.label}</span>
      </button>
    );
  };
      ...
      <div className="flex items-center h-14">
        ...
        <div className="flex items-center justify-center px-2">
          <AddTransaction trigger={
            <motion.button
              whileTap={{ scale: 0.9 }}
              className="w-11 h-11 flex items-center justify-center font-mono"
              style={{
                background: 'hsl(142 60% 52%)',
                color: 'hsl(0 0% 5%)',
                boxShadow: '0 0 16px hsl(142 60% 52% / 0.4)',
              }}
        ...
        <div className="flex flex-1 items-center">
          ...
          <div className="flex flex-col items-center gap-1 flex-1 py-2"
            ...
            <span className="text-[8px] font-mono uppercase tracking-widest">account</span>
          </div>
        </div>
