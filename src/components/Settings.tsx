...
import {
  Lock, RefreshCw, Eye, EyeOff, Copy, Check, AlertTriangle, ChevronRight,
} from 'lucide-react';
import { useEncryption } from '@/components/EncryptionProvider';
import {
  isBiometricAvailable, registerBiometric, getCredentialId, clearCredentialId,
} from '@/lib/webauthn';
import { generatePassphrase, checkStrength } from '@/lib/passphrase';
import {
  deriveKey, base64ToSalt, verifyPassphrase, createVerificationHash,
  generateSalt, saltToBase64,
} from '@/lib/encryption';
  ...
  return (
    <div style={{ borderBottom: '1px solid hsl(0 0% 12%)' }}>
      <div className="px-6 py-5">
        <p className="font-mono text-[9px] uppercase tracking-[0.25em] text-muted-foreground mb-0.5">{title}</p>
        {subtitle && <p className="font-mono text-[11px] text-muted-foreground/60 mb-4">{subtitle}</p>}
...
function Row({
  label, value, action, danger = false, onClick,
}: {
  label: string; value?: React.ReactNode; action?: string;
  ...
  return (
    ...
    >
      <div>
        ...
        {value && <p className="font-mono text-[10px] mt-0.5" style={{ color: 'hsl(0 0% 38%)' }}>{value}</p>}
      </div>
      {action && onClick && (
        <button
          onClick={onClick}
...
function StatusBadge({ on, labels }: { on: boolean; labels: [string, string] }) {
  return (
    <span
      className="font-mono text-[9px] uppercase tracking-widest px-2 py-0.5"
      style={{
        border: on ? '1px solid hsl(142 60% 52% / 0.3)' : '1px solid hsl(0 0% 18%)',
...
function ChangePassphraseModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    if (next.length < 8)   { setError('err: new passphrase must be ≥ 8 chars'); return; }
    if (next !== confirm)  { setError('err: passphrases do not match'); return; }
    setLoading(true);
    try {
      ...
      if (!valid)   { setError('err: current passphrase incorrect'); setLoading(false); return; }

      // Derive new key and save
      const newSalt = await generateSalt();
      const newKey  = await deriveKey(next, newSalt);
      const hash    = await createVerificationHash(newKey);
      await setupMutation({ salt: saltToBase64(newSalt), verificationHash: hash });
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: 'hsl(0 0% 7%)', border: '1px solid hsl(0 0% 16%)', boxShadow: '0 32px 64px hsl(0 0% 0% / 0.6)' }}
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">current passphrase</label>
            <input
              type={show ? 'text' : 'password'}
              value={current} onChange={e => setCurrent(e.target.value)}
              autoFocus className="w-full bg-transparent font-mono text-sm outline-none px-3 py-2.5 text-foreground placeholder:text-muted-foreground/35"
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1 items-center">
                {next && (
                  <button type="button" onClick={handleCopy} className="text-muted-foreground hover:text-foreground transition-colors p-0.5">
                    {copied ? <Check className="h-3 w-3 text-primary" /> : <Copy className="h-3 w-3" />}
                  </button>
                )}
                <button type="button" onClick={() => setShow(!show)} className="text-muted-foreground hover:text-foreground transition-colors p-0.5">
              ...
              <div className="space-y-1.5 mt-2">
                <div className="flex items-center justify-between">
                  <div className="flex gap-1">
                    {[0,1,2,3].map(i => (
                      <div key={i} className="h-0.5 w-8 transition-all duration-300"
                           style={{ background: i < strength.score ? strength.color : 'hsl(0 0% 18%)' }} />
                    ))}
                  </div>
                  <span className="font-mono text-[9px] uppercase tracking-widest" style={{ color: strength.color }}>{strength.label}</span>
                </div>
                {strength.tips[0] && strength.score < 3 && (
                  <p className="font-mono text-[9px]" style={{ color: 'hsl(0 0% 35%)' }}>→ {strength.tips[0]}</p>
                )}
              </div>
            )}
          </div>
          ...
          {/* Confirm */}
          <div className="space-y-1.5">
            <label className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">confirm new passphrase</label>
            <input
              type={show ? 'text' : 'password'}
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
                    ...
                    style={{ border: '1px solid hsl(0 0% 18%)', color: 'hsl(0 0% 42%)', background: 'transparent' }}>
              cancel
            </button>
            <button type="submit" disabled={loading || !current || !next || !confirm}
                    ...
                    style={{ background: 'hsl(142 60% 52%)', color: 'hsl(0 0% 5%)', border: 'none' }}>
              {loading ? <span className="animate-spin">◌</span> : <Lock className="h-3.5 w-3.5" />}
              update
            </button>
          </div>
        </form>
  ...
  return (
    <>
      <div className="min-h-dvh bg-background">
        {/* Page header */}
        <div className="sticky z-20" style={{ top: '48px', borderBottom: '1px solid hsl(0 0% 12%)', background: 'hsl(0 0% 5%)' }}>
          <div className="max-w-2xl mx-auto px-6 py-4">
            <p className="font-mono text-[9px] uppercase tracking-[0.25em] text-muted-foreground mb-1">config</p>
          ...
          >
            ...
            >
              <Row
                label="status"
                value={isEnabled
                  ? isUnlocked ? 'active and unlocked' : 'enabled but locked'
            ...
            <Section
              ...
              subtitle={bioAvail ? 'Touch ID · Face ID · Windows Hello' : 'not supported on this device or browser'}
            >
              {!bioAvail ? (
                <div className="font-mono text-[11px] px-4 py-3"
                     style={{ border: '1px solid hsl(0 0% 13%)', color: 'hsl(0 0% 35%)' }}>
                ...
                <>
                  <Row
                    ...
                    value={
                      <StatusBadge on={bioEnabled} labels={['registered', 'not registered']} />
                    }
                  ...
                  />
                  {bioLoading && (
                    <p className="font-mono text-[9px] uppercase tracking-widest" style={{ color: 'hsl(142 55% 52%)' }}>
            ...
            {isEnabled && (
              <Section title="danger zone">
                <div className="flex items-start gap-2 px-3 py-2.5 font-mono text-[10px]"
                     style={{ border: '1px solid hsl(40 80% 50% / 0.2)', background: 'hsl(40 80% 50% / 0.05)', color: 'hsl(40 75% 60%)' }}>
                  <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />
                  <span>Clearing the local key only locks this device. Encrypted records remain in your database — re-unlock with your passphrase to regain access.</span>
                </div>
                <Row
      ...
      {/* Change passphrase modal */}
      {showChange && <ChangePassphraseModal onClose={() => setShowChange(false)} />}
    </>
  );
}
