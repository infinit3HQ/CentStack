...
export function TransactionList({ limit, compact = false }: TransactionListProps) {
  const allTransactions = useDecryptedTransactions();
  const deleteTransaction = useMutation(api.transactions.remove);
  const transactions = limit && allTransactions ? allTransactions.slice(0, limit) : allTransactions;
  ...
  if (transactions.length === 0) return (
    ...
    >
      <span style={{ color: 'hsl(142 55% 52%)' }}>→ </span>no records found. input required.
    </motion.div>
  );
  ...
  return (
    <div className={cn('pb-20 md:pb-4', compact && 'pb-0')}>
      ...
      {/* ── MOBILE LIST ─────────────────────────────────────────── */}
      <div className="md:hidden divide-y" style={{ borderColor: 'hsl(0 0% 11%)' }}>
        <AnimatePresence mode="popLayout">
          {transactions.map((t, index) => (
            <motion.div
              ...
              onMouseEnter={e => (e.currentTarget.style.background = 'hsl(142 60% 52% / 0.04)')}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-mono text-[12px] truncate text-foreground/90">{t.description}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="font-mono text-[9px] uppercase tracking-wider text-muted-foreground"
                        style={{ borderLeft: '2px solid hsl(0 0% 20%)', paddingLeft: '5px' }}>
                    {t.category}
                  </span>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span
                  ...
                  style={{ color: t.type === 'income' ? 'hsl(142 55% 55%)' : 'hsl(120 3% 75%)' }}
                >
                ...
                {!compact && (
                  <Button
                    ...
                    onClick={e => { e.stopPropagation(); deleteTransaction({ id: t._id as Id<'transactions'> }); }}
                  >
            ...
            {transactions.map((t, index) => (
              ...
              >
                <td className="px-4 py-3 font-mono text-[10px] whitespace-nowrap"
                  ...
                  {new Date(t.date).toLocaleDateString(undefined, { month: '2-digit', day: '2-digit', year: '2-digit' })}
                </td>
                ...
                <td className="px-4 py-3 text-right font-mono text-[13px] num-display"
                    style={{ color: t.type === 'income' ? 'hsl(142 55% 55%)' : 'hsl(120 3% 78%)' }}>
                  {t.type === 'income' ? '+' : '-'}${t.amount.toFixed(2)}
                </td>
